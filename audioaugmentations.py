...
class AudioAugmentations:
    ...
    def __init__(
        ...
        bgNoiseFileList: List[str] = None,  # Add this new parameter for background noise 
        # enhanced_model = None
        ...
        self.speechEnhenceProb = speechEnhenceProb if enableAug else -1

        # self.enhance_model = enhanced_model
    ...
    def loadAudio(
        ...
        # x , sr = librosa.load(path=audioPath,sr=None,mono=True)
    ...
    def getAudio(self, audioPath: str, returnTensor: bool = False) -> Union[np.ndarray, torch.Tensor]:
        ...
        if self.multipleAug == True:         
            ...
            # if random.uniform(0, 1) <= self.speechEnhenceProb:    
            #     wav = self.speech_enhence(wav)
            ...
            # elif aug_choice == "speech_enhence":
            #     wav = self.speech_enhence(wav)
    ...
    def speech_enhence(self, audio_signal: torch.Tensor) -> torch.Tensor:
        
        # for custom file, change path
        model = separator.from_hparams(source="speechbrain/sepformer-wham16k-enhancement", savedir='pretrained_models/sepformer-wham16k-enhancement',run_opts={"device":"cpu"})
        est_sources = model.separate_batch(mix=audio_signal) 
